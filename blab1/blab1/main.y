...
/*终结符 ID*/
%token <type_node> INT_CORRECT FLOAT_CORRECT ID_CORRECT
...
%token <type_node> COMMENT SPACE AERROR FLOAT_ERROR INT_ERROR ID_ERROR

/*非终结符*/
%type <type_node> Program ExtDefList ExtDef ExtDecList ID INT FLOAT
...
OptTag : ID 						{treeNode childList[1] = {$1}; $$= newNode("OptTag", 1, yylineno, childList);nodeList[nodeNum]=$$;nodeNum++;}
       ...
       //| ID_ERROR               {$$= newNode("OptTag", 0, -1, NULL);nodeList[nodeNum]=$$;nodeNum++;char buff[200]={0};sprintf(buff,"不可用的标识符：%s\n",$1->ID) ;yyerror(buff);}
       ;
...
*/
ID : ID_CORRECT                 {}
    | ID_ERROR                  {$$= newNode("OptTag", 0, -1, NULL);nodeList[nodeNum]=$$;nodeNum++;yyerror("不可用的标识符");}
    ;
INT :INT_CORRECT                {}
    |INT_ERROR                 {$$= newNode("OptTag", 0, -1, NULL);nodeList[nodeNum]=$$;nodeNum++;yyerror("不是一个合法的整数");}
    ;
FLOAT: FLOAT_CORRECT            {}
    |FLOAT_ERROR                 {$$= newNode("OptTag", 0, -1, NULL);nodeList[nodeNum]=$$;nodeNum++;yyerror("不是一个合法的浮点数");}
    ;
